...
def train():
    ...
    criteria_scoremap = OhemCELoss(thresh=score_thres, n_min=n_min, ignore_lb=ignore_idx)
    ...
    for it in range(max_iter):
        ...
        if use_boundary_2 and use_boundary_4 and use_boundary_8:
            out, out16, out32, detail2, detail4, detail8, out_scoremap = net(im)
        
        if (not use_boundary_2) and use_boundary_4 and use_boundary_8:
            out, out16, out32, detail4, detail8, out_scoremap = net(im)

        if (not use_boundary_2) and (not use_boundary_4) and use_boundary_8:
            out, out16, out32, detail8, out_scoremap = net(im)

        if (not use_boundary_2) and (not use_boundary_4) and (not use_boundary_8):
            out, out16, out32, out_scoremap = net(im)

        lossp = criteria_p(out, lb)
        loss2 = criteria_16(out16, lb)
        loss3 = criteria_32(out32, lb)
        loss4 = criteria_scoremap(out_scoremap, lb)
        ...
        loss = lossp + loss2 + loss3 + loss4 + boundery_bce_loss + boundery_dice_loss
