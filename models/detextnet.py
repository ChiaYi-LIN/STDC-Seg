...
class DeTextNet(BiSeNet):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False, use_boundary_8=False, use_boundary_16=False, use_conv_last=False, heat_map=False, *args, **kwargs):
        super(DeTextNet, self).__init__(backbone, n_classes, pretrain_model, use_boundary_2, use_boundary_4, use_boundary_8, use_boundary_16, use_conv_last, heat_map, *args, **kwargs)
        self.conv_out = BiSeNetOutput(256 + n_classes, 256, n_classes)
        self.init_weight()
        self.cp = DeTextContextPath(backbone, n_classes, pretrain_model, use_conv_last=use_conv_last)
    ...
    def forward(self, x):
        ...
        feat_out = self.conv_out(torch.cat([feat_fuse, F.interpolate(score_map, feat_fuse.size()[2:], mode='nearest')], dim=1))
